...
pub fn load_db() -> Result<Rdb> {
    ...
    if magic != MAGIC_STRING {
        ...
        println!("Using RDB format version: {}", version);
    ...
    loop {
        ...
        if buf[0] == 0xFA {
            ...
            let mut buf = [0; 1];
            file.read_exact(&mut buf)?;
            let key = extract_value(buf[0], &mut file, ExpectedKind::String)?;

            // Read the next byte, which should be the length of the value
            file.read_exact(&mut buf)?;
            let value = extract_value(buf[0], &mut file, ExpectedKind::String)?;
            ...
            let selected_db = {
                let mut buf = [0; 1];
                file.read_exact(&mut buf)?;
                extract_value(buf[0], &mut file, ExpectedKind::Integer)?
            };
            ...
            file.read_exact(&mut buf)?;
            let db_size = extract_value(buf[0], &mut file, ExpectedKind::Integer)?;
            println!("Database hash table size: {}", db_size);

            file.read_exact(&mut buf)?;
            let expires_size = extract_value(buf[0], &mut file, ExpectedKind::Integer)?;
            println!("Database expiry hash table size: {}", expires_size);
        } else if buf[0] == 0xFF {
            ...
            // Do a key-value pair section

            // Find out if there is an expiry
            let mut buf = [0; 1];
            file.read_exact(&mut buf)?;

            // THE BUG IS HERE - IF THERE IS NO EXPIRY, WE ARE ADVANCING WITHOUT READING THE LENGTH OF THE KEY
            ...
            let key = {
                let mut buf = [0; 1];
                file.read_exact(&mut buf)?;
                extract_value(buf[0], &mut file, ExpectedKind::String)?
            };
...
fn extract_value(byte: u8, file: &mut File, expected: ExpectedKind) -> Result<String> {
    ...
    match nullified {
        0b00000000 => {
            ...
            if expected == ExpectedKind::Integer {
                let val = match length {
                    0 => 0.to_string(),
                    1 => u8::from_le_bytes(val.try_into().unwrap()).to_string(),
                    2 => u16::from_le_bytes(val.try_into().unwrap()).to_string(),
                    4 => u32::from_le_bytes(val.try_into().unwrap()).to_string(),
                    _ => unreachable!(),
                };
                return Ok(val);
            }
        ...
        0b11000011 => {
            let mut buf = [0; 1];
            file.read_exact(&mut buf)?;
            let clen = extract_value(buf[0], file, ExpectedKind::Integer)?.parse::<usize>()?;
